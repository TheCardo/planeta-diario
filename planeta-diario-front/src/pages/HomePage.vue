   ...
   <header class="max-w-7xl mx-auto px-4 pt-6">
    ...
    <main class="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <section class="lg:col-span-8 border-r-0 lg:border-r border-[#1C1C1E]/20 lg:pr-8">
        ...
        <article v-if="topHeadline.titulo" @click="lerNoticia(topHeadline)" class="group cursor-pointer">
      ...
      <aside class="lg:col-span-4">
        ...
        <div class="mt-12 mb-2 border-b-2 border-[#1C1C1E] pb-1">
          <h2 class="text-xs font-black uppercase tracking-widest text-[#0A3161]">Dossiês Recentes</h2>
        </div>
        <div class="flex flex-col gap-4 mt-4">
          <div v-if="dossiesRecentes.length === 0" class="text-[10px] text-black/50 italic">
            Nenhum dossiê publicado ainda.
          </div>
          <article v-for="dossie in dossiesRecentes" :key="dossie.id" @click="abrirDossie(dossie)" class="cursor-pointer group border-b border-[#1C1C1E]/10 pb-4 last:border-0">
            <span class="text-[9px] font-black uppercase tracking-widest text-[#CBA052] mb-1 block">Por {{ dossie.autorNome || dossie.autorUsername }}</span>
            <h4 class="font-serif text-lg font-bold leading-tight group-hover:text-[#0A3161] transition-colors">
              {{ dossie.titulo }}
            </h4>
          </article>
...
// NOVO: Array para guardar os dossiês
const dossiesRecentes = ref([]);
...
const buscarNoticias = async () => {
  ...
  try {
    ...
    topHeadline.value = {
      id: 'offline',
      titulo: '[MODO OFFLINE] GNews API Atinge Limite',
...
const buscarDossiesRecentes = async () => {
  try {
    const res = await api.get('/api/publicacoes/recentes');
    dossiesRecentes.value = res.data;
  } catch (error) {
    console.error("Erro ao buscar dossiês recentes:", error);
  }
};
...
const verificarSessao = async () => {
  if (isLoggedIn.value) {
    try {
      ...
      nomeUsuario.value = response.data.nome || response.data.username;
    } catch (error) {
...
const abrirDossie = (dossie) => {
  const noticiaMock = {
    id: dossie.noticiaUrlRef,
    titulo: "Matéria Arquivada",
    resumo: "Esta notícia não está mais na capa principal. O Dossiê abaixo foi preservado nos Arquivos Confidenciais do Planeta Diário.",
    imagem: "https://images.unsplash.com/photo-1504711434969-e33886168f5c?q=80&w=800&auto=format&fit=crop",
    autor: "Arquivo Central"
  };
  sessionStorage.setItem('noticiaAtual', JSON.stringify(noticiaMock));
  router.push('/noticia');
};
