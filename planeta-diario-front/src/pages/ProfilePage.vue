    ...
    <main v-else-if="usuario" class="max-w-5xl mx-auto px-4 animate-fade-in">
      <section class="flex flex-col md:flex-row gap-8 items-start border-b-2 border-[#1C1C1E] pb-12 mb-8 relative">
        ...
        <div class="absolute top-0 right-0 flex gap-3">
          <button @click="logout" class="text-[10px] font-black uppercase tracking-widest border border-red-600 text-red-600 px-4 py-2 hover:bg-red-600 hover:text-white transition-colors">
            Desconectar
          </button>
          <button @click="abrirModal" class="text-[10px] font-black uppercase tracking-widest border border-[#1C1C1E] px-4 py-2 hover:bg-[#1C1C1E] hover:text-white transition-colors">
            Editar Arquivo
          </button>
        </div>

        <div class="w-32 h-32 md:w-48 md:h-48 flex-shrink-0 mt-8 md:mt-0">
